...
main (int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++)
    {
      ...
      else if (strcmp (arg, "--hell") == 0)
        {
          assert_arg ("--hell");
          do_hell = atoll_b (arg);
          if (do_hell <= 0)
            {
              err (stderr, "invalid number of hell hogs: %lli\n", do_hell);
              exit (1);
            }
        }
  ...
  if (do_cpu + do_io + do_vm + do_hdd + do_hell)
    {
      out (stdout, "dispatching hogs: %lli cpu, %lli io, %lli vm, %lli hdd, %lli hell\n",
           do_cpu, do_io, do_vm, do_hdd, do_hell);
    }
  else
    usage (0);

  /* Round robin dispatch our worker processes.  */
  while ((forks = (do_cpu + do_io + do_vm + do_hdd + do_hell)))
    {
      ...
      if (do_hell)
        {
          switch (pid = fork ())
            ...
            case -1:           /* error */
              ...
              break;
            default:           /* parent */
              dbg (stdout, "--> hoghell worker %lli [%i] forked\n",
                   do_hell, pid);
              ++children;
            }
          --do_hell;
...
usage (int status)
{
  char *mesg =
    ...
    " --hell N        spawn N workers spinning on finding all files of the disk "
